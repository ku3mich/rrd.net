...
namespace RRDNet.Core
{
    ...
    public class Datasource : IRrdUpdatable
    {
        ...
        {
            if (!(updater is Datasource))
            {
                throw new RrdException("Cannot copy Datasource object to " + updater.ToString());
            }
            Datasource datasource = (Datasource)updater;
